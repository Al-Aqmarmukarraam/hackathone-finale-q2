...
export default function VisitSection() {
  return (
    <div>
      ...
      {/* Contact Boxes */}
      <div className="w-full flex flex-wrap justify-center bg-slate-200 py-8">
        {[
          {
            icon: "call",
            title: "Get Support",
            ...
            >
              ...
              <div className="mt-3">
                {emails.map((email, i) => (
                  <h6
                    key={i}
                    className={`${montserrat.className} text-center font-bold text-sm text-black hover:text-blue-500 ${text}`}
                  >
                    {email}
                  </h6>
                ))}
              ...
              <h5
                className={`${montserrat.className} text-center font-bold text-lg mt-3 text-black hover:text-blue-500 ${text}`}
              >
                {title}
              </h5>
              <button className="w-[189px] mt-4 text-myBlue font-bold py-2 px-4 rounded-full outline outline-1 outline-myBlue text-black hover:text-blue-500   ">
                Submit Request
              </button>
            </div>
          )
